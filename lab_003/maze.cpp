...
bool Maze::solve_maze_recursive(int row, int col)
{
	if (row < 0 || col < 0) {
		return false;
	} else if (row >= maze_array_size || col >= maze_array_size) {
		return false;
	}

	if (maze[row][col] == 'X' || maze[row][col] == 'b' || maze[row][col] == '.') {
		return false;
	}
	...
	if (solve_maze_recursive(row - 1, col)) {
		row -= 1;
		return true;
	} else if (solve_maze_recursive(row, col + 1)) {
		//east
		col += 1;
		return true;
	} else if (solve_maze_recursive(row + 1, col)) {
		//south
		row += 1;
		return true;
	} else if (solve_maze_recursive (row, col - 1)) {
...
void Maze::solve_maze_stack(int row, int col)
{
	...
	while (true) {
		...
		// north
		if (row - 1 >= 0 and
                    maze[row-1][col] != 'X' and
                    maze[row-1][col] != 'b' and
                    maze[row-1][col] != '.') {
			stack.push(Position(row, col));
			row -= 1;
			continue;
		} else if (col + 1 < maze_array_size and
			maze[row][col+1] != 'X' and
			maze[row][col+1] != 'b' and
			maze[row][col+1] != '.') {
			//east
			stack.push(Position(row, col));
			col += 1;
...
void Maze::maze_generator(int index, int backtrack_x[CELL], int backtrack_y[CELL],
